...
class ToolService:
    def __init__(self) -> None:
        ...
        # registry
        try:
            from backend.agent_framework.tools.base import (
                ToolRegistry,
                ToolDefinition,
                ToolExecutor,
            )
    ...
    def list_tools(
        ...
        if search:
            items = [
                t
                for t in items
                if search.lower() in (t.get("name") or "").lower()
                or search.lower() in (t.get("description") or "").lower()
            ]
        if agent_id:
        ...
        total = len(items)
        return {
            "tools": items[skip : skip + limit],
            "total": total,
            "categories": list({t.get("category") for t in items}),
        }
            ...
            def execute_fn(p: Dict[str, Any]) -> Any:
                ...
                if handler_url:
                    try:
                        ...
                        r = httpx.request(
                            "POST", handler_url, json=p, timeout=tool["timeout"]
                        )
                        try:
                            return r.json()
                        except Exception:
    ...
    def test_tool(
        self, tool_id: str, input_data: Any, timeout: Optional[int] = None
    ) -> Dict[str, Any]:
        t = self._tools.get(tool_id)
        if not t:
            raise KeyError("not found")
            ...
            if t.get("handler_url"):
                ...
                try:
                    ...
                    r = httpx.post(
                        t.get("handler_url"),
                        json=input_data,
                        timeout=timeout or t.get("timeout", 10),
                    )
        ...
        res = self._executor.execute(
            t.get("name"),
            params=input_data,
            timeout_sec=timeout or t.get("timeout", 10),
        )
        # update stats
        sid = t.get("id")
    ...
    def stats_overview(self) -> Dict[str, Any]:
        ...
        builtin = len(
            [t for t in self._tools.values() if t.get("category") == "builtin"]
        )
...
async def list_tools(
    ...
    res = _svc.list_tools(
        skip=skip, limit=limit, category=category, search=search, agent_id=agent_id
    )
...
async def update_tool(
    request: Request, tool_id: str, payload: Dict[str, Any] = Body(...)
):
    try:
...
async def add_tool_to_agent(
    request: Request, agent_id: str, payload: Dict[str, Any] = Body(...)
):
...
async def list_agent_tools(
    request: Request, agent_id: str, enabled_only: bool = Query(False)
):
